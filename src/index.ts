...
const jwksEndpointToJWKS: Record<string, GetKeyFunction<JWSHeaderParameters, FlattenedJWSInput>> = {};

export function clearCache() {
  for (const key in jwksEndpointToJWKS) delete jwksEndpointToJWKS[key];
}

export function requireAuthorization(f: AzureFunction, options: AuthorizeOptions): AzureFunction {
...
function generateHelpers(context: Context, appInsightsClient?: TelemetryClient) {
  ...
  return {
    validateTokenFromHeader: async (
      ...
      let jwks;
      if (options.jwksEndpoint in jwksEndpointToJWKS) {
        jwks = jwksEndpointToJWKS[options.jwksEndpoint];
      } else {
        jwks = createRemoteJWKSet(new URL(options.jwksEndpoint));
        jwksEndpointToJWKS[options.jwksEndpoint] = jwks;
      }
